...
function Dashboard() {
  ...
  function recordSale(productId, quantity, paymentMethod, customerId) {
    const product = products.find((p) => p.id === Number(productId)), qty = Number(quantity), customer = customers.find((c) => c.id === Number(customerId));
    if (!product) return Alert.alert('Record Sale', 'Please select a product.'); if (!qty || qty <= 0) return Alert.alert('Record Sale', 'Enter a valid quantity.'); if (product.stock < qty) return Alert.alert('Record Sale', `Only ${product.stock} units of ${product.name} are available.`); if (paymentMethod === 'udhaar' && !customer) return Alert.alert('Record Sale', 'Please select a customer for an udhaar sale.');
    const amount = Number(product.sellingPrice) * qty, profit = (Number(product.sellingPrice) - Number(product.purchasePrice)) * qty, timestamp = Date.now();
    set('products', (current) => current.map((p) => p.id === product.id ? { ...p, stock: p.stock - qty } : p)); set('sales', (value) => value + amount);
    if (paymentMethod === 'udhaar') set('customers', (current) => current.map((c) => c.id === customer.id ? { ...c, balance: Number(c.balance || 0) + amount } : c));
    set('transactions', (current) => [{ id: timestamp, type: paymentMethod === 'udhaar' ? 'credit-sale' : 'sale', productName: product.name, quantity: qty, amount, profit, timestamp, customerName: customer?.name || '', note: paymentMethod === 'udhaar' ? `Sale on udhaar to ${customer.name}` : 'Paid sale' }, ...current]); modal('sale', false);
  }
  ...
  function addCustomer(name, phone) { const cleanName = name.trim(); if (!cleanName) return Alert.alert('Add Customer', 'Enter customer name.'); set('customers', (current) => [...current, { id: Date.now(), name: cleanName, phone: phone.trim(), balance: 0 }]); modal('customer', false); }
